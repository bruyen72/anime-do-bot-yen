...
class StreamVideoProcessor {
    ...
    async detectFFmpeg() {
        try {
            ...
            // 1. Tentar ffmpeg.exe local PRIMEIRO
            const localFFmpeg = path.join(__dirname, '..', 'ffmpeg.exe');
            try {
                await this.testFFmpeg(localFFmpeg);
                this.ffmpegPath = localFFmpeg;
                this.hasFFmpeg = true;
                console.log('✅ FFmpeg.exe local detectado:', localFFmpeg);
                return true;
            } catch (e) {
                console.log('⚠️ FFmpeg.exe local não funcionou:', e.message);
            }

            // 2. Tentar ffmpeg-static
            try {
                ...
                if (ffmpegStatic && typeof ffmpegStatic === 'string') {
                    ...
                    this.hasFFmpeg = true;
                    console.log('✅ FFmpeg-static detectado e funcionando');
                    return true;
                } else {
                    console.log('⚠️ FFmpeg-static inválido:', typeof ffmpegStatic);
                }
            } catch (e) {
                console.log('⚠️ FFmpeg-static falhou:', e.message);
            }

            // 3. Tentar FFmpeg do sistema
            try {
                await this.testFFmpeg('ffmpeg');
                this.ffmpegPath = 'ffmpeg';
                this.hasFFmpeg = true;
                console.log('✅ FFmpeg do sistema detectado');
                return true;
            } catch (e) {
                console.log('⚠️ FFmpeg do sistema não disponível:', e.message);
            }
