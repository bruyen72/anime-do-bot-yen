...
class StreamVideoProcessor {
    ...
    async detectFFmpeg() {
        // Tentar primeiro o ffmpeg do sistema
        try {
            await this.testFFmpeg('/usr/bin/ffmpeg');
            this.ffmpegPath = '/usr/bin/ffmpeg';
            this.hasFFmpeg = true;
            console.log('✅ FFmpeg do sistema detectado e funcionando:', this.ffmpegPath);
            return true;
        } catch (systemError) {
            console.log('⚠️ FFmpeg do sistema não encontrado, tentando ffmpeg-static...');
        }

        // Fallback para ffmpeg-static se sistema não funcionar
        try {
            ...
            console.log('❌ Erro fatal ao detectar FFmpeg:', error.message);
            this.hasFFmpeg = false;
            return false;
        }
    }
