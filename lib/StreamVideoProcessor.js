...
class StreamVideoProcessor {
    ...
    async detectFFmpeg() {
        const ffmpegPaths = [
            '/usr/bin/ffmpeg',           // Sistema Linux
            '/usr/local/bin/ffmpeg',     // Sistema macOS
            'ffmpeg'                     // PATH do sistema
        ];

        // Tentar caminhos do sistema primeiro
        for (const path of ffmpegPaths) {
            try {
                await this.testFFmpeg(path);
                this.ffmpegPath = path;
                this.hasFFmpeg = true;
                console.log('✅ FFmpeg do sistema detectado:', this.ffmpegPath);
                return true;
            } catch (error) {
                console.log(`⚠️ FFmpeg não encontrado em ${path}`);
            }
        }

        // Tentar ffmpeg-static com múltiplas estratégias
        const ffmpegStaticStrategies = [
            // Estratégia 1: Carregamento direto
            () => require('ffmpeg-static'),
            
            // Estratégia 2: Caminho atual + node_modules
            () => {
                const path = require('path');
                const ffmpegStaticPath = path.join(process.cwd(), 'node_modules', 'ffmpeg-static');
                return require(ffmpegStaticPath);
            },
            
            // Estratégia 3: Caminho absoluto do projeto
            () => {
                const path = require('path');
                const projectRoot = __dirname.split('/lib')[0]; // Remove '/lib' do caminho
                const ffmpegStaticPath = path.join(projectRoot, 'node_modules', 'ffmpeg-static');
                return require(ffmpegStaticPath);
            },
            
            // Estratégia 4: Busca no sistema de arquivos
            () => {
                const fs = require('fs');
                const path = require('path');
                const possiblePaths = [
                    '/workspace/node_modules/ffmpeg-static',
                ...
                for (const searchPath of possiblePaths) {
                    if (fs.existsSync(searchPath)) {
                        console.log(`🔍 Encontrado ffmpeg-static em: ${searchPath}`);
                        return require(searchPath);
                    }
                }
                throw new Error('ffmpeg-static não encontrado em nenhum caminho');
            }
        ];

        for (let i = 0; i < ffmpegStaticStrategies.length; i++) {
            try {
                console.log(`🔄 Tentando estratégia ${i + 1} do ffmpeg-static...`);
                const ffmpegStatic = ffmpegStaticStrategies[i]();
                
                if (!ffmpegStatic) {
                    throw new Error('ffmpeg-static retornou null/undefined');
                }

                if (typeof ffmpegStatic !== 'string') {
                    throw new Error(`ffmpeg-static retornou tipo ${typeof ffmpegStatic}, esperado string`);
                }
                ...
                console.log(`⚠️ Estratégia ${i + 1} falhou: ${error.message}`);
            }
        }

        // Se tudo falhar, tentar usar ffmpeg.exe local (caso Windows)
        try {
            const path = require('path');
            ...
            if (fs.existsSync(localFFmpeg)) {
                console.log('⚠️ Encontrado ffmpeg.exe local, mas precisamos de Wine para executar no Linux');
                console.log('💡 Considere instalar: sudo apt install wine');
            }
        } catch (e) {
            // Ignorar erro
        }

        console.log('❌ Nenhum FFmpeg encontrado. Funcionará apenas com fallback visual.');
        this.hasFFmpeg = false;
        return false;
    }
